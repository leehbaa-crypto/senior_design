...
def main(args=None):
    ...
    try:
        ...
        with mujoco.viewer.launch_passive(model, data) as viewer:
            print("🚀 Active Node v12 Running: Pull-Back Avoidance (No Camera Vis)")
            ...
            while viewer.is_running():
                ...
                if not l_avoid_active and not r_avoid_active and not f_active:
                    ...
                    # 💡 [핵심 변경 1] 도망칠 때는 원래 위치로 돌아가려는 힘을 대폭 줄여서 뒤로 쭉 빠지게 만듭니다.
                    attract_gain = 0.05 if is_dodging else 0.5 
                    ...
                    if f_active:
                        ...
                        # 💡 [핵심 변경 2] 도망칠 때 자세 제어(Elbow-Up)를 끄는 대신 오히려 가중치를 3배로 높여서 팔꿈치를 들고 뒤로 당기도록 강제합니다.
                        posture_gain = 1.5 if is_dodging else 0.5 
                    ...
                    for i, j_idx in enumerate(joint_ids):
                        ...
                        if is_dodging: filtered_dq[i] = np.clip(filtered_dq[i], -0.1, 0.1) 
